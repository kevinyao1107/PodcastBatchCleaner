...
- 可切換使用外部 AI 模型命令處理音檔，再接續 FFmpeg 後製流程
- 可為每個音檔設定剪輯開始與結束時間
- 可將多段錄音依清單順序統整成單一音檔
- 可設定段落間空白秒數與統整輸出檔名
...
5. 如果要讓程式呼叫本機 AI 模型，勾選「使用 AI 模型命令處理」，選擇 AI 命令，並確認參數包含 `{input}` 與 `{output}`。
6. 可先套用「一般 Podcast」「空間音重」「聲音偏薄」「只剪輯」模板，再微調靜音秒數、靜音門檻、降噪、減少空間音、人聲增厚 EQ、響度標準化、防爆音、音量增益與輸出格式。
7. 如需剪輯，可在清單中的「開始」「結束」欄位輸入時間。
8. 如需改檔名，可在清單中的「輸出檔名」欄位輸入新檔名。
9. 如需合併多段錄音，可用「上移」「下移」調整清單順序，並勾選「統整成單一檔案」。
10. 統整輸出時可視需要填寫標題、作者、節目名稱，並選擇封面圖片。
11. 可視需要選擇片頭、片尾，並調整淡入淡出秒數。
12. 點選「處理選取」或「全部輸出」。
13. 開始處理前，程式會先檢查 FFmpeg、輸出資料夾、檔案路徑、剪輯時間與片頭片尾/封面路徑。
14. 處理時可在進度視窗查看目前檔案與百分比，也可按「取消」中止。
15. 處理結束後會顯示摘要；一般批次中單一檔案失敗時，程式會繼續處理後面的檔案。
16. 可使用「開啟輸出」「開原檔位置」「開處理後檔」快速查看檔案。
...
如果有本機 AI 模型或第三方工具可以用命令列處理音檔，可勾選「使用 AI 模型命令處理」。程式會先執行 AI 命令，產生暫存 WAV，再接續原本的 FFmpeg 後製、剪輯、輸出格式與統整輸出流程。
...
也可以選擇 `.exe`、`.cmd`、`.bat`、`.ps1` 或 `.py`。Python 腳本需要系統能找到 `python` 命令。
