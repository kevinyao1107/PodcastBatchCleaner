...
7. 如需快速確認參數效果，選取單一音檔後點選「30秒試聽」。
8. 如需比較不同處理效果，選取單一音檔後點選「A/B 比較」。
9. 如需剪輯，可在清單中的「開始」「結束」欄位輸入時間。
10. 如需改檔名，可在清單中的「輸出檔名」欄位輸入新檔名。
11. 如需合併多段錄音，可用「上移」「下移」或拖曳清單列調整順序，並勾選「統整成單一檔案」。
12. 統整輸出時可視需要填寫標題、作者、節目名稱，並選擇封面圖片。
13. 可視需要選擇片頭、片尾，並調整淡入淡出秒數。
14. 點選「處理選取」或「全部輸出」。
15. 開始處理前，程式會先檢查 FFmpeg、DeepFilterNet、輸出資料夾、檔案路徑、剪輯時間與片頭片尾/封面路徑。
16. 處理時可在進度視窗查看目前檔案、處理階段與百分比，也可按「取消」中止。
17. 處理結束後會顯示摘要；一般批次中單一檔案失敗時，程式會繼續處理後面的檔案。
18. 可使用「開啟輸出」「開原檔位置」「開處理後檔」快速查看檔案。
